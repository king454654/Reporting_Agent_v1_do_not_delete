...
# --- NEW HELPER FUNCTION ---
# Note: Requires 'openpyxl' to be installed (pip install openpyxl)
...
if "report_focus" not in st.session_state:
    st.session_state.report_focus = "Pacing Report" # Default example
        ...
        if not (ws and db and tbl):
            ...
            try:
                with st.spinner("Generating report... (fetching schema, sampling data, calling LLM)"):
                    ...
                    st.session_state.last_report_df = df # Save for export
                    st.session_state.last_report = "" # Clear old report
