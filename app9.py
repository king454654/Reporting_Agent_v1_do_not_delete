...
def fetch_filtered_data(workspace, database, table, col_map, start_dt, end_dt, limit=500000):
    ...
    # FIX 1: Wrap column names in backticks for the SELECT statement
    ...
    # FIX 2: Wrap date column in backticks for WHERE clause
...
def generate_excel_report(raw_df, summary_kpi_df, report_dfs, col_map, global_budget_val) -> bytes:
    ...
    with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
        ...
        def get_unique(col_key):
            ...
            # Check if column exists in DataFrame (Pandas cols do not have backticks)
                ...
                with st.spinner("Processing..."):
                    try:
                        ...
                        if df_filtered.empty:
                            ...
                            # --- FIXED: Escaped columns for aggregation queries ---
                            total_budget_global = run_aggregation_query(ws, db, tbl, f"`{spend_col}`", "SUM") 
