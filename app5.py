...
def send_email_with_attachment(recipient_email, excel_bytes, filename="pacing_report.xlsx"):
    ...
    try:
        ...
        body = "Hello,\n\nPlease find the attached Pacing Report generated via Databricks.\n\nBest Regards,\nAnalytics Team"
...
def process_granular_data(df, cols_map, freq='W'):
    ...
    # This handles cases where there are multiple rows (campaigns) per day
    ...
    # 3. RESAMPLE
    if freq == 'D':
        # For Daily, we already have the data, just ensure index is reset
    ...
    for _, row in grouped.iterrows():
        ...
        # Skip padding rows if completely empty
...
if "daily_df" not in st.session_state: st.session_state.daily_df = None # NEW
    ...
    if col_names:
                ...
                with st.spinner("Crunching numbers..."):
                    try:
                        ...
                        if df_filtered.empty:
                            ...
                            daily_df = process_granular_data(df_filtered, col_map, freq='D') # NEW
                            ...
                            st.session_state.excel_bytes = to_excel(dfs_to_save)
        ...
        if st.session_state.excel_bytes is not None:
            ...
            with tab2:
                ...
                st.dataframe(
                    st.session_state.daily_df.style.format({"Budget": "${:,.2f}", "Spend": "${:,.2f}", "Revenue": "${:,.2f}", "ROAS": "{:.2f}"}), 
            ...
            with tab3:
                ...
                st.dataframe(
                    st.session_state.weekly_df.style.format({"Budget": "${:,.2f}", "Spend": "${:,.2f}", "Revenue": "${:,.2f}", "ROAS": "{:.2f}"}), 
                    use_container_width=True
                )
            ...
            with tab4:
                ...
                st.dataframe(
                    st.session_state.monthly_df.style.format({"Budget": "${:,.2f}", "Spend MTD": "${:,.2f}", "Revenue MTD": "${:,.2f}", "ROAS": "{:.2f}"}), 
                    use_container_width=True
                )
