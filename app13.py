...
def safe_identifier(col_name):
    """Wraps column names in backticks to handle spaces and special chars like /"""
...
def get_dynamic_insights(df, period_type):
    """Generates top 5 textual insights based on the dataframe."""
    if df.empty:
        return ["No data available to generate insights."]
    
    insights = []
    
    # Normalize column names for logic (Monthly has 'MTD')
    ...
    # 1. Identify Top Performer (ROAS)
    if 'ROAS' in df.columns:
        # Check if ROAS column has valid data
        if df['ROAS'].sum() > 0:
    ...
    # 4. Trend (Last Period vs Previous) - Only if we have > 1 row
    ...
    return insights[:5] # Strict limit to 5 lines

# ---------- EXCEL GENERATION (Original - No Logo) ----------
...
def generate_excel_report(raw_df, summary_kpi_df, report_dfs, col_map, global_budget_val) -> bytes:
    ...
    with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
        ...
        # ==========================================
        # 2. GRANULAR SHEETS WITH INSIGHTS
        # ==========================================
        for sheet_name, df in report_dfs.items():
            ...
            if not df.empty:
                ...
                ws.set_column('K:K', 50) # Widen column for text
                ...
                # Chart Logic (Shifted to Column L)
                ...
                if col_idx_spend >= 0 and col_idx_budget >= 0:
                    ...
                    chart1.set_x_axis({'major_gridlines': {'visible': False}})
                    chart1.set_y_axis({'major_gridlines': {'visible': False}})
                    chart1.set_size({'width': 600, 'height': 300})
                    # Insert Chart at L2 (Right of insights)
                ...
                if col_idx_rev >= 0 and col_idx_roas >= 0:
                    ...
                    chart2.set_y2_axis({'name': 'ROAS', 'major_gridlines': {'visible': False}})
...
def send_email_with_attachment(recipient_email, excel_bytes, filename="pacing_report.xlsx"):
    ...
    try:
        ...
        body = "Hello,\n\nPlease find the attached Pacing Report with Insights.\n\nBest Regards,\nAnalytics Team"
...
# ================================
# LOGO LOGIC (Only in Application)
# ================================
if os.path.exists(LOGO_FILENAME):
    st.sidebar.image(LOGO_FILENAME, width=200) # Adjust width as needed
# ================================
...
if st.session_state.selected_table:
    ...
    if col_names:
        ...
        if st.session_state.excel_bytes:
            ...
            # --- UI Formatting for Currency ---
            currency_format = st.column_config.NumberColumn(format="$%.2f")
            cols_config = {
                "Budget": currency_format,
                "Spend": currency_format,
                "Spend MTD": currency_format,
                "Revenue": currency_format,
                "Revenue MTD": currency_format
            }
            # ----------------------------------
