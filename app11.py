...
def run_aggregation_query(workspace, database, table, col_def_sql, agg_type="SUM"):
    try:
        ...
        # col_def_sql must already be escaped by the caller if it's a simple column
...
# ---------- Data Fetching (FIXED FOR SPECIAL CHARS) ----------
...
def fetch_filtered_data(workspace, database, table, col_map, start_dt, end_dt, limit=500000):
    ...
    # Wrap date column in backticks for WHERE clause
...
# ---------- EXCEL GENERATION (CLEAN - NO GRIDLINES - WITH CURRENCY) ----------
...
def generate_excel_report(raw_df, summary_kpi_df, report_dfs, col_map, global_budget_val) -> bytes:
    ...
    with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
        ...
        # New format for data columns with currency
...
if st.session_state.selected_table:
    ...
    if col_names:
        ...
        if st.session_state.excel_bytes:
            ...
            with t2: st.dataframe(st.session_state.daily_df, column_config=cols_config, use_container_width=True)
            with t3: st.dataframe(st.session_state.weekly_df, column_config=cols_config, use_container_width=True)
            with t4: st.dataframe(st.session_state.monthly_df, column_config=cols_config, use_container_width=True)
